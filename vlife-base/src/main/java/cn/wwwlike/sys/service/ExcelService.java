...
public class ExcelService {
     ...
     * 导出指定实体excel模版
     */
    public Workbook template(FormDto formVo) {
        ...
        Sheet sheet = workbook.createSheet(formVo.getTitle());
        ...
        //不导入的字段过滤
        List<FormFieldDto> fields = formVo.getFields().stream().filter(
...
//                && f.x_hidden!=true  //直接隐藏
    ...
    // 通过中文名称获取表头字段的模型信息
    public List<FormFieldDto> getExcelFields(List<FormFieldDto> all, Workbook workbook) throws IOException {
        List<FormFieldDto> list = new ArrayList<>();
        Row titleRow = title(workbook);
        for (Cell cell : titleRow) {
            Optional<FormFieldDto> optional = all.stream().filter(f -> f.getTitle().trim().equals(cell.getStringCellValue().trim())).findFirst();
     ...
     * excel数据转实体数据
     */
    public List<DbEntity> excelToItems(FormDto form, Workbook workbook, boolean override, DataImpResult result) throws IOException, InstantiationException, IllegalAccessException, ParseException {
        List<FormFieldDto> fields = getExcelFields(form.getFields(), workbook);
        Class itemClazz = GlobalData.entityDto(form.getEntityType()).getClz();
        List list = excelToList(itemClazz, fields, workbook);
        List<? extends DbEntity> excelItems = listToItems(list, fields);
     ...
     * excel数据解析
     */
    private List<Item> excelToList(Class<Item> entityClz, List<FormFieldDto> fields, Workbook workbook) throws InstantiationException, IllegalAccessException, IOException, ParseException {
        Sheet sheet = workbook.getSheetAt(0);
        ...
        for (Row row : sheet) {
            ...
            if (row.getRowNum() > 1) {
                for (Cell cell : row) {
                    ...
                    FormFieldDto _field = fields.get(columiIndex);
     ...
     * 提取数据转实体数据
     */
    private List<Item> listToItems(List<Item> list, List<FormFieldDto> fields) {
    ...
    //单个实体覆盖
    public Item  override(Item item,Item db,List<FormFieldDto> fields) {
        fields.forEach(f->{
    ...
    //必填数据过滤
    private List<?extends Item> innerRequiredFilter(List<? extends Item> excelItems,List<FormFieldDto> requiredFields){
        //excel数据去重
        for(FormFieldDto f:requiredFields){
    ...
    //excel数据内部查重，去除重复数据
    private List<?extends Item> innerRepeatFilter(List<? extends Item> excelItems,List<FormFieldDto> uniqueFields){
        //excel数据去重
        for(FormFieldDto f:uniqueFields){
            List list=new ArrayList();
            Set set=new HashSet();
            for(Item i:excelItems){
                Object val=ReflectionUtils.getFieldValue(i,f.getFieldName());
     ...
     */
    public List<? extends Item> overrideImport(List<? extends Item> items,List<FormFieldDto> fields,Class<Item> entityClz, DataImpResult result){
        List<Item> dbItems = new ArrayList<>();
        //唯一的字段
        List<FormFieldDto> uniqueFields = fields.stream().filter(field -> field.isValidate_unique()==true).collect(Collectors.toList());
        List<FormFieldDto> requiredFields = fields.stream().filter(field -> field.isRequired()==true).collect(Collectors.toList());
...
//                   item覆盖db(clone到db的id)
     ...
     */
    public List<? extends Item> skipImport(List<? extends Item> excelItems,List<FormFieldDto> fields,Class<Item> entityClz, DataImpResult result){
        //唯一的字段
        List<FormFieldDto> uniqueFields = fields.stream().filter(field -> field.isValidate_unique()==true).collect(Collectors.toList());
        List<FormFieldDto> requiredFields = fields.stream().filter(field -> field.isRequired()==true).collect(Collectors.toList());
        //必填数据为空排除
    ...
    public void save(List<DbEntity> items, String entityClz){
