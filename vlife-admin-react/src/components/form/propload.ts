          ...
          if (prop.propVal && prop.propName) {
            if (prop.sourceType === sourceType.api) {
              ...
              if  (prop.sourceType==="api"&& apiInfo &&prop.relateVal&& (allParam===undefined||hasAllValues(paramObj,allParam))){
                ...
                propsObj = await loadApi({apiInfoKey:prop.propVal,paramObj:{...paramObj,fieldVal,...fieldOutParams},match:prop.relateVal,filterConnectionType:prop.filterConnectionType,filterFunc:prop.filterFunc}).then((d) => {
