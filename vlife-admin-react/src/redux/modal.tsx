...
export const modalReducer = (
  state = { hiding: {}, activeModalNum: undefined },
  action: any
) => {
  switch (action.type) {
 ...
 * @returns
 */
function NiceModal({ id, children, width, size, ...rest }: NiceModelProps) {
...
export const createNiceModal = (modalId: string, Comp: any) => {
  ...
  return (props: any) => {
    // props 是初始参数 layout里使用
    const { visible, args } = useNiceModal(modalId); //纳入到store管理
    if (!visible) {
      //它会在对话框不可见时直接返回 null，从而不渲染任何内容；并且确保即使页面上定义了 100 个对话框，也不会影响性能：
      return null;
    }
    //args 组件核心方法
