 ...
 * 日期数据转换成年度(整型)
 */
public  class WeekExpressTran extends DataExpressTran<SimpleExpression,NumberExpression> {
    public  NumberExpression tran(SimpleExpression expression,String dbType) {
        if(expression instanceof DatePath)
            return ((DatePath)expression).week();
        else
            return ((DateTimePath)expression).week();
    }
}
