...
public class QueryUtils {
     ...
     * @return
     */
    public static <T extends AbstractWrapper> AbstractWrapper conditions(AbstractWrapper qw, Conditions condition) {
            ...
            qw.and(ww -> where((T) ww, condition.getWhere())
                    .and(condition.conditions != null, (d -> {
                        for (Conditions cc : condition.getConditions()) {
                            ((AbstractWrapper<?, ?, ?>) ww).and(ee -> conditions((T) ee, cc));//递归
                        }
                    })));
        }
    ...
    /**
     * 条件查询设计器转换成conditions(复杂的查询结构)
     * @param json
     * @return
     */
    public static Conditions conditionJsonToConditions(String json) {
      List<ConditionGroup> conditionGroups=  JSON.parseArray(json,ConditionGroup.class);
      Conditions conditions=new Conditions();
      conditions.setOrAnd("or");
      conditions.setConditions(new ArrayList<>());
      for(ConditionGroup group:conditionGroups){
          Conditions sub=new Conditions();
          sub.setOrAnd("and");
          sub.setWhere(group.getWhere());
          conditions.getConditions().add(sub);
      }
      return conditions;
    }

    //将多组查询条件组合在一起
    public static Conditions conditionsJoin(String orAnd,List<Conditions> conditions) {
        Conditions _conditions=new Conditions();
        _conditions.setOrAnd(orAnd);
        _conditions.setConditions(conditions);
      return _conditions;
    }



//    public static Conditions conditionGroupToConditions(List<ConditionGroup> conditionGroups, Optional<Object> formData) {
//        List<Condition> conditions = conditionGroups.stream()
//                .map(group -> {
//                    List<Object> whereClause;
//                    if (formData.isPresent()) {
//                        whereClause = group.where.stream()
//                                .map(w -> where(w, formData.get()))
//                                .collect(Collectors.toList());
//                    } else {
//                        whereClause = group.where;
//                    }
//                    return new Condition("and", whereClause);
//                })
//                .collect(Collectors.toList());
//        return new Conditions("or", new ArrayList<>(), conditions);
//    }
     ...
     * 组合组是or连接
     */
    public static <T extends AbstractWrapper> T conditionGroup(T qw, List<ConditionGroup> groups) {
    ...
    //一组where条件植入(简单)
    public static <T extends AbstractWrapper> T conditionWhere(T qw, List<List<Where>> groups) {
        qw.or(ww -> groups.forEach(g ->
                ((T) ww).and(www -> where((T) www, g))
        ));
        return qw;
    }
