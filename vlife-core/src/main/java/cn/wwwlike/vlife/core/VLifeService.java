...
import static cn.wwwlike.vlife.dict.VCT.MODEL_TYPE;
...
public class VLifeService<T extends Item, D extends VLifeDao<T>> {
     ...
     * 不进行权限过滤的包装查询？
     */
    public List<T> findWithoutPermissions(QueryWrapper<T> qw) {
        return dao.find(qw);
    }
    /**
     * 7.条件包装对象查询总数
    ...
    public <E extends IdBean> List<E> query(Class<E> clz, Conditions conditions) {
        ...
        QueryUtils.conditions(request.qw(), conditions);
    ...
    public <E extends VoBean<T>, S extends QueryWrapper> List<E> queryAll(Class<E> vo) {
        QueryWrapper<T> wrapper = QueryWrapper.of(entityClz);
        return dao.query(vo,wrapper, null);
    }
     ...
     * 15 批量逻辑删除
     */
    public long batchRm(String... ids) throws Exception{
        if (ids == null || ids.length == 0) {
     ...
     * 删除逻辑，递归逻辑
     */
    private long remove(Class<? extends Item> itemClz, String... ids) throws Exception {
//        String[] ids = id.split(",");
...
//                    return -1; //没有给出原因
        ...
        if (ids.length > 1) {
            ...
            return dao.remove(itemClz,  ids[0]);
        }
    }
    ...
    protected <E extends IdBean> E saveBean(final E beanDto, Class<? extends Item> fkItemClz, String fkItemId, UnaryOperator<DataProcess> callBackMethod, boolean subIsFull) {
        ...
        if (beanDto instanceof SaveBean) {
            ...
            //1.外键表
            saveDto.filter(MODEL_TYPE.ENTITY, MODEL_TYPE.DTO).stream().filter(
                    fieldDto -> {
            ...
            //3. 关联数据保存
            saveDto.filter(MODEL_TYPE.ENTITY, MODEL_TYPE.VO, VCT.ITEM_TYPE.LIST).stream().filter(
                    fieldDto -> {
                        Class fkClz = fieldDto.getEntityClz();
            ...
            saveDto.filter(VCT.ITEM_TYPE.LIST).stream().filter(
                    fieldDto -> {
                        Class fkClz = fieldDto.getEntityClz();
