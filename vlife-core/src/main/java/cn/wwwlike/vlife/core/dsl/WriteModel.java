...
@Getter
public class WriteModel<T extends DbEntity> implements WModel<T> {
    ...
    public WriteModel(JPAQueryFactory factory, Class<? extends IdBean> saveBeanClz) {
        this.factory=factory;
        this.entityClz = DbEntity.class.isAssignableFrom(saveBeanClz)?(Class<T>) saveBeanClz :GenericsUtils.getGenericType(saveBeanClz);
        main = getItemEntityPath(entityClz);
        this.saveClz = saveBeanClz;
        this.updateClause = factory.update(main);
        this.saveDto =DbEntity.class.isAssignableFrom(saveBeanClz)?GlobalData.entityDto(entityClz): GlobalData.saveDto((Class<SaveBean>)saveBeanClz);
    }
