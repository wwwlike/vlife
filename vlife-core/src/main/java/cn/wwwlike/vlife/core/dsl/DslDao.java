...
import static cn.wwwlike.vlife.utils.VlifeUtils.convertEmptyStringsToNull;
...
//    }
    ...
    public <E extends SaveBean> E save(E saveBean, Map<String, Object> fkMap) {
        convertEmptyStringsToNull(saveBean);
        ...
        if (saveBean.getId() != null) {
            ...
            fkMap.forEach((k, v) -> {
                ...
                ReflectionUtils.setFieldValue(saveBean, k, v);
            });
            save(item);
            saveBean.setId(item.getId());
        }
        //将fkMap 中的值赋给 saveBean
