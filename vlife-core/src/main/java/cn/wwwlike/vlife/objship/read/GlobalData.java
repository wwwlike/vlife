...
public class GlobalData {
    ...
    public static BeanDto findBeanDtoByName(String itemType,String modelName) {
        ...
        if (VCT.MODEL_TYPE.DTO.equals(itemType)) {//数据编辑用途
            list =GlobalData.getSaveDtos().values();
        } else if (VCT.MODEL_TYPE.VO.equals(itemType)) {// 数据展示用途
            list = GlobalData.getVoDtos().values();
        } else if (VCT.MODEL_TYPE.REQ.equals(itemType)) { //数据查询用途
            list = GlobalData.getReqDtos().values();
        }else if(VCT.MODEL_TYPE.ENTITY.equals(itemType)){
            list = GlobalData.getEntityDtos().values();
        }else if(VCT.MODEL_TYPE.BEAN.equals(itemType)){
            list = GlobalData.getBeanDtos().values();
        }else if(itemType==null){
            String[] itemTypes={VCT.MODEL_TYPE.REQ,VCT.MODEL_TYPE.DTO,VCT.MODEL_TYPE.ENTITY,VCT.MODEL_TYPE.VO,VCT.MODEL_TYPE.BEAN};
