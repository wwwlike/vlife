...
public class SysUserService extends BaseService<SysUser, SysUserDao> implements UserDetailsService {
    ...
    public PehrSecurityUser getPerhSecurityUser(String username){
        List<SysUser> users=find("username",username);
        if(users==null||users.size()==0){
            throw new UsernameNotFoundException(username+"is not exist");
        }else{
     ...
     * @return
     */
    public PehrSecurityUser createUserFromGitee(JSONObject object){
        PehrSecurityUser user=new PehrSecurityUser();
//        user.setUsername();
//        user.setSysOrgId();
//        user.setSysDeptId();
//        user.setSysGroupId();
